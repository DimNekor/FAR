...
class ResultsScreen(Screen):
    ...
    def refresh_results(self):
        ...
        for header_text, header_width in headers:
            ...
            with header_label.canvas.before:
                ...
                Color(0.2, 0.2, 0.3, 1)
                header_label.bg_rect = Rectangle(pos=header_label.pos, size=header_label.size)
        ...
        for i, row in enumerate(results):
            ...
            predicted_text = "Реальное" if row['predicted_class'] == 1 else \
                           "Фейк" if row['predicted_class'] == 0 else "Таймаут"
            
            is_correct = row['true_class'] == row['predicted_class']
            status = "✓ Верно" if is_correct else "✗ Ошибка" if row['predicted_class'] != -1 else "⏱ Таймаут"
    ...
    def show_export_dialog(self):
        ...
        filename_box = BoxLayout(orientation="horizontal", size_hint=(1, 0.1), spacing=10)
        filename_box.add_widget(Label(
            text="Имя файла:",
            size_hint=(0.3, 1)
        ))
        ...
        # Кнопки
        buttons_box = BoxLayout(orientation="horizontal", size_hint=(1, 0.1), spacing=10)
    ...
    def _export_to_excel(self, filepath):
        ...
        try:
            ...
            for row in results:
                ...
                predicted_text = "Реальное" if row['predicted_class'] == 1 else \
                               "Фейк" if row['predicted_class'] == 0 else "Таймаут"
                is_correct = "Да" if row['true_class'] == row['predicted_class'] else \
                           "Таймаут" if row['predicted_class'] == -1 else "Нет"
            ...
            self._show_popup("Успех", f"Данные успешно экспортированы в файл:\n{filepath}")
